...
use crate::model_portfolio::TaxOptimizationSettings;
use crate::portfolio_rebalancing::{RebalanceTrade, TradeReason};
use crate::error::{ModelPortfolioErrorType};
...
impl AlgorithmicTLHService {
    ...
    /// Adjust the minimum loss percentage based on market volatility
    fn adjust_for_market_volatility(&self, volatility: Option<f64>) -> f64 {
        let mut adjusted_min_loss_percentage = self.config.min_loss_percentage;
        
        // If volatility is provided, adjust the minimum loss percentage
        ...
        adjusted_min_loss_percentage
    }

    /// Find eligible tax lots for harvesting
    fn find_eligible_lots(&self, uma: &UnifiedManagedAccount) -> Vec<(String, TaxLot)> {
        let mut eligible_lots = Vec::new();
        
        for sleeve in &uma.sleeves {
            for holding in &sleeve.holdings {
                for lot in &holding.tax_lots {
                    // Check if the lot meets the criteria for tax-loss harvesting
                    if self.is_eligible_for_harvest(&holding.security_id, lot) {
                        eligible_lots.push((sleeve.id.clone(), lot.clone()));
                    }
                }
            }
        }
    ...
    pub fn generate_real_time_tlh_trades(
        ...
        let mut adjusted_min_loss_percentage = self.adjust_for_market_volatility(market_volatility);
    ...
    /// Generate a TLH performance report
...
pub struct TLHPerformanceReport {
    ...
    pub average_recovery_time_days: u32,
    /// Number of times wash sale prevention was effective
    pub wash_sale_prevention_count: usize,
} 